...
//   // Get store data
//   const { storeData, loading: storeLoading } = useStore('sam-s-store');
...
//   const storeLogo = storeData?.logoUrl || "/images/bakery/bakery-logo.jpg";
...
//                 {storeLoading ? (
//                   <span className="inline-block w-48 h-8 bg-gray-200 animate-pulse rounded" />
//                 ) : (
...
const Header: React.FC = () => {
  ...
  const handleProceedToCheckout = () => {
    ...
    console.log('Proceeding to checkout with items:', cartItems);
    toast.success(`Proceeding to checkout with ${cartItems.length} item${cartItems.length > 1 ? 's' : ''}`);
    setIsCartModalOpen(false);
    router.push('/checkout');
  };
